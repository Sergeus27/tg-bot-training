...
func (s *Storage) PickRandom(userName string) (*storage.Page, error) {
	q := `SELLECT url FROM pages WHERE user_name = ? ORDER BY RANDOM() LIMIT 1`
...
//IsExists checks if page existed in storage. 
func (s *Storage)IsExist(ctx context.Context, page *storage.Page)(bool,error){
	q:=`SELECT COUNT(*) FROM pages WHERE url=? AND user_name=?`

	err:= s.db.QueryRowContext(ctx, q, page.URL, page.UserName).Scan(&count); err!=nil{
...
func (s *Storage)Init(ctx context.Context) error{
	...
	if 	err!=nil{
		return fmt.Errorf("can't create tabler: %w",err )
	}
	return nil
}